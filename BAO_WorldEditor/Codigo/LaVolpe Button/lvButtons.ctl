...
Private Sub DrawGradient(ByVal Color1 As Long, ByVal Color2 As Long)
   ...
   On Error GoTo DrawGradient_Error

10    mRect.Right = ScaleWidth
20    mRect.Bottom = ScaleHeight
30    rctOffset = 1
40    If myProps.bGradient < 3 Then
50            If (Screen.Width \ Screen.TwipsPerPixelX) \ ScaleWidth < 4 Then
60                PixelStep = ScaleWidth \ 2
70                rctOffset = 2
80            Else
90                PixelStep = ScaleWidth
100           End If
110   Else
120       If (Screen.Height \ Screen.TwipsPerPixelY) \ ScaleHeight < 4 Then
130           PixelStep = ScaleHeight \ 2
140           rctOffset = 2
150       Else
160           PixelStep = ScaleHeight
170       End If
180   End If
190   ReDim Colors(0 To PixelStep - 1) As Long
200   LoadGradientColors Colors(), Color1, Color2
210   If myProps.bGradient > 2 Then mRect.Bottom = rctOffset Else mRect.Right = rctOffset
220   For i = 0 To PixelStep - 1
230       If myProps.bGradient Mod 2 Then rIndex = i Else rIndex = PixelStep - i - 1
240       DrawRect ButtonDC.hdc, mRect.Left, mRect.Top, mRect.Right, mRect.Bottom, Colors(rIndex)
250       If myProps.bGradient > 2 Then
260           OffsetRect mRect, 0, rctOffset
270       Else
280           OffsetRect mRect, rctOffset, 0
290       End If
300   Next
...
DrawGradient_Error:
    ...
    Call LogError("Error " & err.Number & " (" & err.Description & ") en procedimiento DrawGradient de Control de usuario lvButtons_H línea: " & Erl())
End Sub
...
10    Tag = UserControl.ContainerHwnd
20    With myProps
30        .bCaption = Ambient.DisplayName
40        .bCaptionAlign = vbCenter
50        .bShowFocus = True
60        .bForeHover = vbButtonText
70        .bBackHover = vbButtonFace
80    End With

90    If Not (TypeOf Parent Is MDIForm) Then Set UserControl.Font = Parent.Font
100   On Error GoTo 0
110   cParentBC = ConvertColor(Ambient.BackColor)
120   curBackColor = vbButtonFace         ' this will be the button's initial backcolor
130   GetGDIMetrics "BackColor"
140   PropertyChanged "Caption"
150   PropertyChanged "CapAlign"
160   PropertyChanged "Focus"
170   PropertyChanged "cFHover"
180   PropertyChanged "cBHover"
190   PropertyChanged "Font"
...
UserControl_InitProperties_Error:
    ...
    Call LogError("Error " & err.Number & " (" & err.Description & ") en procedimiento UserControl_InitProperties de Control de usuario lvButtons_H línea: " & Erl())
End Sub
...
10    Tag = UserControl.ContainerHwnd
20    cParentBC = ConvertColor(Ambient.BackColor)
30    DelayDrawing True
40    With PropBag
50        myProps.bCaption = .ReadProperty("Caption", "")
60        myProps.bCaptionAlign = .ReadProperty("CapAlign", 2)
70        myProps.bBackStyle = .ReadProperty("BackStyle", 0)
80        myProps.bShape = .ReadProperty("Shape", 0)
90        myProps.bGradient = .ReadProperty("Gradient", 0)
100       myProps.bGradientColor = .ReadProperty("cGradient", vbButtonFace)
110       UserControl.ForeColor = .ReadProperty("cFore", vbButtonText)
120       Set UserControl.Font = .ReadProperty("Font")
      '    If UserControl.FontName = "Webdings" Then Stop
130       myProps.bShowFocus = .ReadProperty("Focus", True)
140       myProps.bMode = .ReadProperty("Mode", 0)
150       myProps.bValue = .ReadProperty("Value", False)
160       myProps.bCustomClick = .ReadProperty("CustomClick", 0)
170       Set myImage.Image = .ReadProperty("Image", Nothing)
180       myImage.Size = .ReadProperty("ImgSize", 16)
190       myImage.Align = .ReadProperty("ImgAlign", 0)
200       myProps.bForeHover = .ReadProperty("cFHover", vbButtonText)
210       UserControl.Enabled = .ReadProperty("Enabled", True)
220       curBackColor = .ReadProperty("cBack", Parent.BackColor)
230       myProps.bBackHover = .ReadProperty("cBHover", curBackColor)
240       myProps.bLockHover = .ReadProperty("LockHover", 0)
250       myProps.bCaptionStyle = .ReadProperty("CapStyle", 0)
260       Set Me.MouseIcon = .ReadProperty("mIcon", Nothing)
270       Me.MousePointer = .ReadProperty("mPointer", 0)
280   End With

290   GetGDIMetrics "Picture"
300   GetGDIMetrics "BackColor"
310   Me.Caption = myProps.bCaption      ' sets the hot key if needed
320   If myProps.bMode = lv_OptionButton Then ToggleOptionButtons (1)
330   bNoRefresh = False
340   Call UserControl_Resize
...
UserControl_ReadProperties_Error:
    ...
    Call LogError("Error " & err.Number & " (" & err.Description & ") en procedimiento UserControl_ReadProperties de Control de usuario lvButtons_H línea: " & Erl())
End Sub
