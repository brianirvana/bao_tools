...
Begin VB.Form frmOptimizar 
   ...
   Begin VB.CheckBox chkVerGrhs 
      Caption         =   "Borrar Grhs inexistentes"
      Height          =   495
      Left            =   120
      TabIndex        =   8
      Top             =   2640
...
Dim J                           As Long
    ...
    For Y = YMinMapSize To XMaxMapSize
        For X = XMinMapSize To YMaxMapSize
            ...
            If chkVerGrhs.Value = 1 Then
                For i = 1 To 4
                    If MapData(X, Y).Graphic(i).GrhIndex Then
                        If GrhData(MapData(X, Y).Graphic(i).GrhIndex).NumFrames = 0 Then
                            MapData(X, Y).Graphic(i).GrhIndex = 0
                            MapData(X, Y).Graphic(i).FrameCounter = 0
                            MapData(X, Y).Graphic(i).Started = 0
                            MapData(X, Y).Graphic(i).SpeedCounter = 0
                            J = J + 1
                        End If
                    End If
                Next i
    ...
    If chkVerGrhs.Value = 1 Then
        MsgBox "Se han borrado " & J & " Grhs que no existian."
    End If
