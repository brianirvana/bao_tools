...
Static UltimoY                  As Integer
    ...
    If UltimoX = SobreX And UltimoY = SobreY Then
        Exit Sub
    End If
...
Public Sub PegarSeleccionInvertida()    '(mx As Integer, my As Integer)
...
Static UltimoY                  As Integer
    ...
    For X = DeSeleccionAncho - 1 To 0 Step -1
        For Y = DeSeleccionAlto - 1 To 0 Step -1
            DeSeleccionMap(X, Y) = MapData(X + SobreX, Y + SobreY)
        Next
    Next
    
    For X = SeleccionAncho - 1 To 0 Step -1
        For Y = SeleccionAlto - 1 To 0 Step -1
            MapData(X + SobreX, Y + SobreY) = SeleccionMap(X, Y)
        Next
    Next
